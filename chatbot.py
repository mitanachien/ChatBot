...
# Sting Encoding
threshold = 20  # hyperparameter to tune
questionswords2int = {}
word_number = 0
...
def preprocess_targets(targets, word2int, batch_size):
    ...
    right_side = tf.strided_slice(targets, [0, 0], [batch_size, -1], [1, 1]) # Extract a subset of tensor
    preprocessed_targets = tf.concat([left_side, right_side], axis = 1) # axis: horizontal = 1
...
def decode_training_set(encoder_state, decoder_cell, decoder_embedded_input, sequence_length, decoding_scope, 
    ...
    attention_states = tf.zeros([batch_size, 1, decoder_cell.output_size]) # Initiate 3 dimentional matrix
...
def seq2seq_model(inputs, targets, keep_prob, batch_size, sequence_length, answers_num_words, questions_num_words, 
    ...
    # Looks up embeddings for the preprocessed_targets from decoder_embeddings_matrix
    ...
    for batch_index, (padded_questions_in_batch, padded_answers_in_batch) in enumerate(split_into_batches(training_questions, 
        ...
        if batch_index % batch_index_check_validation_loss == 0 and batch_index > 0:
            ...
            if average_validation_loss_error <= min(list_validation_loss_error):
                print("I speak beter now!")
                early_stopping_check = 0
                saver = tf.train.Saver()
                saver.save(session, checkpoint)
            else:
                print("Sorry Ido not speak better, I need to practice more.")
